...
void cpush_c_(int& n,int& ns)
{
    ...
    for(m == 1; m < mm[ns]; m++)
    {
        ...
        wz1 = 1.-wz0;
        dbdrp   = wx0*wz0*dbdr[i][k]+wx0*wz1*dbdr[i][k+1]+wx1*wz0*dbdr[i+1][k]+wx1*wz1*dbdr[i+1][k+1];
        dbdtp   = wx0*wz0*dbdth[i][k]+wx0*wz1*dbdth[i][k+1]+wx1*wz0*dbdth[i+1][k]+wx1*wz1*dbdth[i+1][k+1];
        grcgtp  = wx0*wz0*grcgt[i][k]+wx0*wz1*grcgt[i][k+1]+wx1*wz0*grcgt[i+1][k]+wx1*wz1*grcgt[i+1][k+1]; 
        bfldp   = wx0*wz0*bfld[i][k]+wx0*wz1*bfld[i][k+1]+wx1*wz0*bfld[i+1][k]+wx1*wz1*bfld[i+1][k+1];
        radiusp = wx0*wz0*radius[i][k]+wx0*wz1*radius[i][k+1]+wx1*wz0*radius[i+1][k]+wx1*wz1*radius[i+1][k+1];
        dydrp   = wx0*wz0*dydr[i][k]+wx0*wz1*dydr[i][k+1]+wx1*wz0*dydr[i+1][k]+wx1*wz1*dydr[i+1][k+1];
        qhatp   = wx0*wz0*qhat[i][k]+wx0*wz1*qhat[i][k+1]+wx1*wz0*qhat[i+1][k]+wx1*wz1*qhat[i+1][k+1];
        grp     = wx0*wz0*gr[i][k]+wx0*wz1*gr[i][k+1]+wx1*wz0*gr[i+1][k]+wx1*wz1*gr[i+1][k+1];
        gxdgyp  = wx0*wz0*gxdgy[i][k]+wx0*wz1*gxdgy[i][k+1]+wx1*wz0*gxdgy[i+1][k]+wx1*wz1*gxdgy[i+1][k+1];       
        curvbzp = wx0*wz0*curvbz[i][k]+wx0*wz1*curvbz[i][k+1]+wx1*wz0*curvbz[i+1][k]+wx1*wz1*curvbz[i+1][k+1];
        bdcrvbp = wx0*wz0*bdcrvb[i][k]+wx0*wz1*bdcrvb[i][k+1]+wx1*wz0*bdcrvb[i+1][k]+wx1*wz1*bdcrvb[i+1][k+1];
        grdgtp  = wx0*wz0*grdgt[i][k]+wx0*wz1*grdgt[i][k+1]+wx1*wz0*grdgt[i+1][k]+wx1*wz1*grdgt[i+1][k+1];       
        ...
        ydot = (-exp1/b+vpar/b*delbyp)*dum1*nonlin[ns]+iorb*enerb/bfldp/bfldp*fp/radiusp*grcgtp*(-dydrp*dbdtp+r0/q0*qhatp*dbdrp)+vp0+enerb/(bfldp**2)*psipp*lr0/q0/radiusp**2*(dbdrp*grp**2+dbdtp*grdgtp)-mims[ns]*vpar**2/(q[ns]*bstar*b)*(psip2p*grp**2/radiusp+curvbzp)*lr0/(radiusp*q0)-dipdrp/radiusp*mims[ns]*vpar**2/(q[ns]*bstar*b)*grcgtp*lr0/q0*qhatp;  
        zdot =  vpar*b/bstar*(1.-tor+tor*q0*br0/radiusp/b*psipp*grcgtp)/jfnp+q0*br0*enerb/(b*b)*fp/radiusp*dbdrp*grcgtp/jfnp-1./b**2*q0*br0*fp/radiusp*grcgtp*vncp*vexbsw/jfnp-dipdrp/radiusp*mims[ns]*vpar**2/(q[ns]*bstar*b)*q0*br0*grcgtp/jfnp;     
        pzd0 = tor*(-mu[ns][m]/mims[ns]/radiusp/bfldp*psipp*dbdtp*grcgtp)*b/bstar+mu[ns][m]*vpar/(q[ns]*bstar*b)*dipdrp/radiusp*dbdtp*grcgtp;
        pzdot = pzd0 + (q[ns]/mims[ns]*ezp*q0*br0/radiusp/b*psipp*grcgtp/jfnp+q[ns]/mims[ns]*(-xdot*delbyp+ydot*delbxp+zdot*dadzp))*ipara;        
        edot = q[ns]*(xdot*exp1+(ydot-vp0)*eyp+zdot*ezp)+q[ns]*pzdot*aparp*tor+q[ns]*vpar*(-xdot*delbyp+ydot*delbxp+zdot*dadzp)-q[ns]*vpar*delbxp*vp0;      
        ...
        if(ildu == 1)
        {
             dum = (tgis[ns]/ter)^1.5*exp(vfac*(1/tgis[ns]-1./ter));
        }
        ...
        if(abs(w3[ns][m])>1.0 && nonlin[ns]==1)
        {
           w3[ns][m] = 0.;
           w2[ns][m] = 0.;
        }     

        laps= int((z3[ns][m]/lz)-.5)*(1-peritr);
        ...
        y3[ns][m]= int(y3[ns][m]-laps*2*pi*qr*lr0/q0*sign(1.0,q0)+8000.*ly)%int(ly);

        if(x3(ns,m)>lx && iperidf==0)
        {
           x3[ns][m] = lx-1.e-8;
           z3[ns][m]=lz-z3(ns,m);
           x2[ns][m] = x3(ns,m);
